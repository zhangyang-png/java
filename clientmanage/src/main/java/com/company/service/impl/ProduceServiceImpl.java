...
public class ProduceServiceImpl implements IProduceService {
	...
	public void change(List<Produce> list,List<ProduceVO> volist){
		for (Produce produce : list) {
			...
			ProduceVO vo = new ProduceVO(produce.getId(),produce.getName(),produce.getPrice(),produce.getKind(),kindname,produce.getDesc(),produce.getCount(),produce.getImgStr(),produce.getStatusid());
			volist.add(vo);
		}
	...
	@Override
	public boolean addProduce(String name,String price,int kind,String desc,int count,String imgStr) {
		return produceDao.addProduce(name,price,kind,desc,count,imgStr)>0?true:false;
	}

	@Override
	public boolean updateProduce(int id,String name,String price,int kind,String desc,int count,String imgStr) {
		return produceDao.updateProduce(id,name,price,kind,desc,count,imgStr)>0?true:false;
	}
